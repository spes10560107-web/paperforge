...
本研究旨在建立一套以深度學習影像辨識為核心的 3D 列印件自動瑕疵辨識與品質評分系統，以魚骨形狀 PLA 列印件為研究標的，聚焦於拉絲瑕疵嚴重程度的六等級分類（A 至 F）。資料蒐集主要集中於 2025/11/22 至 2025/12/10，並於 2026/02/05 至 2026/02/07 補充少數類別與失敗件樣本，並於 2026/02/10 前完成資料裁切、標注、模型訓練與辨識分析。本研究資料集共取得 1110 筆有效魚骨樣本，包含 A 級 546 張、B 級 87 張、C 級 213 張、D 級 160 張、E 級 72 張、F 級 32 張。

模型架構選用 MobileNetV3-Large（ImageNet V2 預訓練權重），以遷移學習方式進行六分類微調，並附加品質評分輔助頭同時輸出 0 至 100 的連續品質分數。本研究先以 Focal Loss（$\gamma = 2.0$）、Label Smoothing（0.1）、批次層級 Mixup（$\alpha=0.3$）、Weighted Random Sampler 過採樣、Cosine Annealing Warm Restarts 與 Early Stopping 等完整策略為原始候選設定；後續補充實驗進一步完成 Group Split、五個 seed 重複訓練、單變因消融、等預算比較、校準分析、Grad-CAM、PR/ROC 與外部未標注影像 sanity check，以區分單次 checkpoint 表現與可重複結論。

實驗結果方面，**Group Split 嚴格測試集（168 筆、與訓練集零原圖重疊）之 seed=7 簡化 MobileNetV3-Large 整體準確率為 86.31\%（145/168）、macro-F1 = 0.791、weighted-F1 = 0.859、QWK = 0.932**，各等級為 A 級 100.0\%（77/77）、B 級 56.3\%（9/16）、C 級 77.1\%（27/35）、D 級 82.1\%（23/28）、E 級 75.0\%（6/8）、F 級 75.0\%（3/4）。然而五個 seed（7、42、123、1234、2024）重複後，MobileNetV3-Large 簡化、MobileNetV3-Large 完整、ResNet18 完整、EfficientNet-B0 完整之平均準確率分別為 81.19\%、80.12\%、80.83\%、81.19\%，相對簡化模型之 paired t-test 在 accuracy 與 QWK 皆未達顯著差異（所有 $p > 0.5$）；macro-F1 則以完整 MobileNetV3-Large 與 ResNet18 較高（相對簡化之 $p=0.0246$ 與 $p=0.0363$）。因此本文將 86.31\% 視為 seed=7 詳細分析案例，而非「簡化策略穩定勝出」之定論。

**本研究貢獻為 FDM 魚骨拉絲瑕疵小樣本影像分類提供可重現基準，並呈現一項較保守但更穩健的結論**：在本資料規模下，單次 seed=7 排名容易受切分與訓練隨機性影響；五個 seed 後，架構與訓練策略在 accuracy / QWK 上尚無穩定顯著差異，但完整策略與 ResNet18 對 macro-F1 及跨等級誤判控制較有優勢。校準分析顯示簡化模型 ECE 由 0.0576 經 temperature scaling 降至 0.0435，高信心（$>0.9$）子集準確率 95.5\%，低信心（$<0.7$）子集準確率僅 46.4\%，可作為人工複核觸發依據。本系統已完整整合自動裁切、批量辨識、分級輸出與 HTML 可視化報告；惟受限於單一機型（Bambu Lab A1）、單一材料（PLA）、單一形狀（魚骨）、單一瑕疵類型（拉絲）與尚未具標注答案之外部 OOD 測試集，現階段仍定位為品質管控輔助原型，而非可獨立取代人工判定之系統。
...
2. **嚴格分組切分與多 seed 泛化評估**：在以原圖為單位之 Group Split（seed=7、訓練 / 驗證 / 測試各 774 / 168 / 168 筆、原圖零重疊）下，MobileNetV3-Large 簡化設定達整體準確率 **86.31\%（145/168）、macro-F1 = 0.791**；進一步以五個 seed 重複後，簡化 MobileNetV3-Large 之平均準確率為 81.19 $\pm$ 3.71\%、macro-F1 為 0.661 $\pm$ 0.079、QWK 為 0.921 $\pm$ 0.009，提供比單次切分更保守的小樣本工業視覺基準。
...
4. **端對端批量辨識系統可行性**：整合自動裁切、批量推論、分級輸出與 HTML 可視化報告，主要簡化模型在 NVIDIA GeForce RTX 3050 Laptop GPU 上單張推論時間約 6.90 ms（約 144.9 張/秒），驗證在桌上型 GPU 上之離線部署可行性。
...
4. **資料相關性與分組切分**：1110 張樣本係由 185 張原始照片各裁切出約 6 支魚骨而來，同一原圖切出之多支樣本在光線、背景與列印批次上高度相關。為驗證同源相關性對泛化能力的實際影響，本研究已採用 \texttt{GroupShuffleSplit}（以原圖 ID 為 group 鍵，seed=7）重新訓練並在零原圖重疊的嚴格測試集（168 筆）上評估；補充實驗中 MobileNetV3-Large 簡化設定達 86.31\%（145/168），高於隨機切分 version\_21 的 83.23\%（詳見 \ref{sec:results-supplemental} 節）。此結果顯示模型在嚴格分組切分下仍具可行性，但 B、E、F 測試樣本數偏少（16、8、4 張），仍需視為初步觀察而非穩定結論。
5. **標注一致性未驗證**：全部 1110 張樣本由單一標注者依固定準則完成，現階段無法邀請第二位以上標注者重新標注，因此未進行多人標注一致性測試（如 Cohen's Kappa、Fleiss' Kappa 或 ICC）。「品質等級制度」一語應理解為「單一標注者依明確規則建立之可重複準則」，而非多人標注的一致客觀標準；模型表現亦應解讀為相對此準則之學習結果。
6. **品質分數未經人工校準**：0 至 100 分輸出採等級編號之線性轉換為監督目標，尚未與多位有經驗操作人員之主觀評分對齊；現階段僅作為分類結果之輔助連續指標，不宜作為絕對品質量化依據。
7. **基準模型比較與消融研究之範圍限制**：本研究已補充五 seed baseline、單變因消融與等預算比較，包含 MobileNetV3-Large 簡化 / 完整、ResNet18 完整與 EfficientNet-B0 完整設定。結果顯示「單次 seed=7 最佳模型」與「五 seed 平均後的穩定結論」並不完全一致，因此本文不再將簡化策略宣稱為穩定最佳；目前可較有把握的結論是 accuracy / QWK 差異未達顯著，而完整策略或 ResNet18 對 macro-F1 較有利。
8. **Ordinal 任務以 nominal classification 近似處理**：A 至 F 為有序等級（ordinal label），本研究以 Cross Entropy / Focal Loss 視為六類獨立分類，搭配「等級編號線性轉換」(A=100, B=80, ..., F=0) 之 MSE 輔助頭隱式建模順序關係。本文已補充 QWK、Cohen's Kappa、$\pm 1$ 容差與跨等級誤判率，但尚未採用 ordinal-aware loss（如 CORAL、CORN），因此模型訓練目標仍非真正的 ordinal regression。
9. **模型決策可解釋性僅完成定性檢查**：本研究已補充 Grad-CAM 正確 / 錯誤樣本視覺化，作為模型是否關注魚骨主體與拉絲區域的定性檢查；但因缺乏像素級拉絲標註或人工 attention ground truth，Grad-CAM 不能被解讀為定量因果證明。
10. **拍攝條件單一，OOD 僅完成未標注 sanity check**：訓練 / 測試樣本皆於「黑色 PEI 熱床 + 室內自然光 + 30 cm 俯拍 + 智慧型手機」之單一條件採集。本文已對專案內 \texttt{待辨識/} 105 張外部照片切出 630 支魚骨並統計預測分布與 confidence，但該批資料無人工標籤，因此只能檢查輸出分布與低信心比例，不能作為 OOD accuracy。
11. **文獻與外部資料來源之準確性限制**：本文引用來源依用途區分為三類：(a) 標準、教科書與同儕審查論文，用於定義 FDM、CNN、遷移學習與模型架構等方法背景；(b) 印表機或切片軟體廠商文件，用於說明常見列印瑕疵與實務調參經驗；(c) 市場研究報告，用於描述產業規模與應用脈絡。第二、三類來源具有實務參考價值，但可能隨產品版本、廠商文件更新或市場估計方法改變而變動，因此不作為本文實驗結論之核心證據。本文引用鍵與參考文獻條目另透過 PaperForge 編譯流程（Pandoc、biblatex 與 biber）檢查其對應關係與格式可解析性[@kevin00156Kevin00156Paperforge2026]；惟此類機械檢查僅能確認引用是否可編譯，不能保證外部網頁永久可用、廠商文件完全中立，或市場報告估計必然準確。本文所有模型效能、資料分布、混淆矩陣與統計檢定，均以本研究本機資料與可重現程式輸出為準。
...
\hline
\multicolumn{2}{>{\centering\arraybackslash}p{10.6cm}}{\textit{＝＝ Group Split 補充實驗\\（測試集 168 筆，與訓練集零原圖重疊）＝＝}} \\
\hline
